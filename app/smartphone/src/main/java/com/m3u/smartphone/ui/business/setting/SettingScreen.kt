...
import com.m3u.smartphone.benchmark.DevDefaults
...
fun SettingRoute(
    ...
    LaunchedEffect(Unit) {
        if (DevDefaults.XTREAM_BASIC_URL != null) {
            viewModel.properties.selectedState.value = DataSource.Xtream
            viewModel.properties.titleState.value = DevDefaults.XTREAM_TITLE.orEmpty()
            viewModel.properties.basicUrlState.value = DevDefaults.XTREAM_BASIC_URL.orEmpty()
            viewModel.properties.usernameState.value = DevDefaults.XTREAM_USERNAME.orEmpty()
            viewModel.properties.passwordState.value = DevDefaults.XTREAM_PASSWORD.orEmpty()
            viewModel.subscribe()
        }
    }
