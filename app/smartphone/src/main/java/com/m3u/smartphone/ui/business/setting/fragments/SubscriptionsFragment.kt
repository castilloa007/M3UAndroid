...
private fun SettingProperties.applyDevXtreamPrefill() {
    DevDefaults.XTREAM_TITLE?.let { titleState.value = it }
    DevDefaults.XTREAM_BASIC_URL?.let { basicUrlState.value = it }
    DevDefaults.XTREAM_USERNAME?.let { usernameState.value = it }
    DevDefaults.XTREAM_PASSWORD?.let { passwordState.value = it }
}
...
private fun XtreamInputContent(modifier: Modifier = Modifier) {
    ...
    LaunchedEffect(Unit) {
        properties.applyDevXtreamPrefill()
    }
